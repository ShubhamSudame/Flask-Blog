...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and hashing.check_value(user.password, form.password.data) == True:
...
def new_post():
    ...
    if form.validate_on_submit():
        post = Post(title=form.title.data,
                    content=form.content.data, author=current_user.username)
        db.session.add(post)
        db.session.commit()
...
def send_reset_email(user):
    ...
    message.body = f'''To reset your password, visit the following link: {url_for('reset_token', token=token, _external=True)}. If you did not make this request then simply ignore this email and no changes will be made.'''
    mail.send(message)
